...
class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.navBg = this.add.rectangle(0, 0, width * z, 70, 0x3e2723, 0.9).setOrigin(0).setScrollFactor(0);
        this.coinText = this.add.text(20, 20, `💰 Coins: ${this.coins}`, { fontSize: '22px', fill: '#ffd54f', fontStyle: 'bold' }).setScrollFactor(0);
        this.levelText = this.add.text((width * z) - 170, 20, `⭐ Level: ${this.level}`, { fontSize: '22px', fill: '#fff', fontStyle: 'bold' }).setScrollFactor(0);
        this.expBar = this.add.rectangle((width * z) - 170, 50, 0, 10, 0x4caf50).setOrigin(0).setScrollFactor(0);

        this.shopBtn = this.add.text((width * z) / 2 - 130, 35, "🛒 SHOP", { fontSize: '18px', backgroundColor: '#4e342e', padding: 8 }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
        this.pauseBtn = this.add.text((width * z) / 2, 35, "⏸ PAUSE", { fontSize: '18px', backgroundColor: '#d84315', padding: 8 }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
        this.menuBtn = this.add.text((width * z) / 2 + 130, 35, "📋 MENU", { fontSize: '18px', backgroundColor: '#4e342e', padding: 8 }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
        ...
        // --- SHOP & MENU LOGIC ---
        this.shopContainer = this.add.container(0, 0).setScrollFactor(0).setDepth(2000).setScale(uiScale).setVisible(false);
        this.shopElements = [];
        const shopBg = this.add.rectangle((width * z) / 2, (height * z) / 2, 450, 420, 0x3e2723).setStrokeStyle(4, 0x795548).setScrollFactor(0);
        const shopTitle = this.add.text((width * z) / 2, (height * z) / 2 - 170, "UPGRADE SHOP", { fontSize: '28px', fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0);
        ...
        const addItem = (foodKey, cost, reqLevel, yOffset) => {
            const bg = this.add.rectangle((width * z) / 2, (height * z) / 2 + yOffset, 400, 55, 0x4e342e).setInteractive({ useHandCursor: true }).setScrollFactor(0);
            const icon = this.add.image((width * z) / 2 - 160, (height * z) / 2 + yOffset, foodKey).setScale(1.8).setScrollFactor(0);
            const name = foodKey.split('_')[1].toUpperCase();
            const text = this.add.text((width * z) / 2 - 120, (height * z) / 2 + yOffset, `Unlock ${name} (${cost} Coins) [REQ LV ${reqLevel}]`, { fontSize: '13px' }).setOrigin(0, 0.5).setScrollFactor(0);
        ...
        const closeBtn = this.add.text((width * z) / 2, (height * z) / 2 + 110, " [ CLOSE ] ", { fontSize: '24px', backgroundColor: '#ff5252', padding: 10 }).setOrigin(0.5).setInteractive({ useHandCursor: true }).setScrollFactor(0);
        ...
        this.menuContainer = this.add.container(0, 0).setScrollFactor(0).setDepth(2000).setScale(uiScale).setVisible(false);
        this.menuElements = [];
        const menuBg = this.add.rectangle((width * z) / 2, (height * z) / 2, 500, 400, 0x2d1b18).setStrokeStyle(4, 0x8d6e63).setScrollFactor(0);
        const menuTitle = this.add.text((width * z) / 2, (height * z) / 2 - 160, "OUR MENU", { fontSize: '32px', fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0);
        const closeMenuBtn = this.add.text((width * z) / 2, (height * z) / 2 + 160, " [ BACK ] ", { fontSize: '24px', backgroundColor: '#5d4037', padding: 8 }).setOrigin(0.5).setInteractive({ useHandCursor: true }).setScrollFactor(0);
        ...
        this.menuBtn.on('pointerdown', () => {
            ...
            this.foodOptions.forEach((food, index) => {
                const y = (height * z) / 2 - 80 + (index * 60);
                const icon = this.add.image((width * z) / 2 - 180, y, food).setScale(2).setScrollFactor(0);
                const text = this.add.text((width * z) / 2 - 140, y, `${food.split('_')[1].toUpperCase()} - 💰${this.foodPrices[food]}`, { fontSize: '20px' }).setOrigin(0, 0.5).setScrollFactor(0);
        ...
        this.pauseContainer = this.add.container(0, 0).setScrollFactor(0).setDepth(10000).setScale(uiScale).setVisible(false);
        this.pauseElements = [];
        let pBg = this.add.rectangle((width * z) / 2, (height * z) / 2, 450, 380, 0x4e342e, 0.95).setStrokeStyle(5, 0xf48fb1).setScrollFactor(0);
        let pTitle = this.add.text((width * z) / 2, (height * z) / 2 - 120, "KAFE DI-ISTIRAHATKAN", { fontSize: '28px', fontStyle: 'bold', fill: '#fff3e0', fontFamily: 'Courier New' }).setOrigin(0.5).setScrollFactor(0);
        let btnResume = this.add.text((width * z) / 2, (height * z) / 2 - 25, "  🌸 KEMBALI BEKERJA  ", { fontSize: '18px', fill: '#ffffff', backgroundColor: '#f48fb1', padding: 12, fontFamily: 'Courier New', fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
        let btnRestart = this.add.text((width * z) / 2, (height * z) / 2 + 40, "  🔄 ULANG HARI INI  ", { fontSize: '18px', fill: '#4e342e', backgroundColor: '#ffe0b2', padding: 12, fontFamily: 'Courier New', fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
        let btnExit = this.add.text((width * z) / 2, (height * z) / 2 + 105, "  🚪 PULANG KE MENU   ", { fontSize: '18px', fill: '#ffffff', backgroundColor: '#880e4f', padding: 12, fontFamily: 'Courier New', fontStyle: 'bold' }).setOrigin(0.5).setScrollFactor(0).setInteractive({ useHandCursor: true });
